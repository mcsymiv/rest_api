...
namespace RestTest.Steps
{
    ...
    public class ApiTestSteps
    {
        ...
        {
            UserData.Add("email", $"{randomString}@mail.com");
            UserData.Add("name", $"{randomString}");
        ...
        {
            request = new RestRequest("tasks/rest/createuser", Method.POST);
            request.RequestFormat = DataFormat.Json;
        ...
        public void ThenIGetBackUserInformationInTheResponseBody()
        {
            var temp = response.Content;
            JObject json = JObject.Parse(temp);
            Assert.AreEqual($"{randomString}@mail.com".ToLower(), json["email"].ToString());
            Assert.AreEqual(randomString, json["name"].ToString());
        }
        ...
        public void GivenATaskDataIsGenerated()
        {
            ...
            TaskData.Add("task_description", randomString);
